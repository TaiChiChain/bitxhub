...
type ChainIndexerBackend interface {
	...
	Process(ctx context.Context, header *types.BlockHeader) error
...
type ChainIndexerChain interface {
	...
	SubscribeBlockEvent(ch chan<- events.ExecutedEvent) event.Subscription
}
...
type ChainIndexer struct {
	chainDb  *ledger.Ledger            // Chain database to index the data from
	indexDb  *storagemgr.CachedStorage // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb *ledger.Ledger, indexDb *storagemgr.CachedStorage, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
		...
		logger:      loggers.Logger(loggers.Indexer),
	}
...
func (c *ChainIndexer) Start(chain ChainIndexerChain) {
	events := make(chan events.ExecutedEvent, 10)
	sub := chain.SubscribeBlockEvent(events)
...
// queue.
func (c *ChainIndexer) eventLoop(currentHeader *types.BlockHeader, events chan events.ExecutedEvent, sub event.Subscription) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if header.ParentHash != prevHash {
				...
				// TODO: if prevhash is not consistent
			}
			c.newHead(header.Number, false)

			_, prevHash = header, header.Hash()
		}
	}
}
	...
	if head >= c.confirmsReq {
		...
		if sections > c.knownSections {
			if c.knownSections < c.checkpointSections {
				...
				syncedHead, err := c.chainDb.ChainLedger.GetBlockHeader(c.checkpointSections*c.sectionSize - 1)
				if err != nil {
					{
						c.logger.Error("Checkpoint height block header not found", "height", c.checkpointSections*c.sectionSize-1)
					}
				}
				if syncedHead.Hash() != c.checkpointHead {
					c.logger.Error("Synced chain does not match checkpoint", "number", c.checkpointSections*c.sectionSize-1, "expected", c.checkpointHead, "synced", syncedHead)
					return
				}
			}
		...
		case <-c.update:
			...
			if c.knownSections > c.storedSections {
				...
				if err == nil && (section == 0 || oldHead == c.SectionHead(section-1)) {
					...
					// c.logger.Info("setValidSections", section+1)
					c.setValidSections(section + 1)
					...
					for _, child := range c.children {
						c.logger.Debug("Cascading chain index update", "head", c.cascadedHead)
...
func (c *ChainIndexer) processSection(section uint64, lastHead *types.Hash) (*types.Hash, error) {
	c.logger.Debug("Processing new chain section", "section", section)
	...
	for number := section * c.sectionSize; number < (section+1)*c.sectionSize; number++ {
		header, err := c.chainDb.ChainLedger.GetBlockHeader(number)
		if err != nil {
			return &types.Hash{}, fmt.Errorf("block #%d not found", number)
		}
...
func (c *ChainIndexer) verifyLastHead() {
	for c.storedSections > 0 && c.storedSections > c.checkpointSections {
		header, err := c.chainDb.ChainLedger.GetBlockHeader(c.storedSections*c.sectionSize - 1)
		if err != nil {
			return
		}
		if equal, _ := header.Hash().Equals(c.SectionHead(c.storedSections - 1)); equal {
			return
		}
...
func (c *ChainIndexer) SectionHead(section uint64) *types.Hash {
	...
	hash := c.indexDb.Get(append([]byte("shead"), data[:]...))
