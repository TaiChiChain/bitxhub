...
func Test_KV_Compatibility(t *testing.T) {
	...
	for name, tc := range testcase {
		t.Run(name, func(t *testing.T) {
			...
			testChainLedger_OpenStateDB(t, tc.kvType)
...
func testChainLedger_OpenStateDB(t *testing.T, kv string) {
	repoRoot := t.TempDir()

	logger := log.NewWithModule("opendb_test")
	ss, err := OpenStateDB(filepath.Join(repoRoot, kv), "simple", kv)
	assert.Nil(t, err)
	blockFile, err := blockfile.NewBlockFile("", logger)
	assert.NotNil(t, err)
	var blockStorage storage.Storage
	if kv == "leveldb" {
		blockStorage, err = leveldb.New(filepath.Join(repoRoot, "ledger", kv))
		assert.Nil(t, err)
	} else if kv == "pebble" {
		blockStorage, err = pebble.New(filepath.Join(repoRoot, "ledger", kv))
		assert.Nil(t, err)
	}
	accountCache, err := NewAccountCache()
	assert.Nil(t, err)
	_, err = New(createMockRepo(t), blockStorage, ss, blockFile, accountCache, log.NewWithModule("executor"))
	assert.Nil(t, err)
}
