...
func (t *testExecTask) Hash() types.Hash {
	return *types.NewHash([]byte(fmt.Sprintf("%d", t.txIdx)))
}
...
func TestZeroTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address { return *types.NewAddress(common.BigToAddress(big.NewInt(int64(1))).Bytes()) }
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestAlternatingTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address {
			return *types.NewAddress(common.BigToAddress(big.NewInt(int64(i % 2))).Bytes())
		}
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestRandomTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		...
		sender := func(i int) types.Address {
			return *types.NewAddress(common.BigToAddress(big.NewInt(int64(rand.Intn(10)))).Bytes())
		}
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestDexScenario(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address { return *types.NewAddress(common.BigToAddress(big.NewInt(int64(i))).Bytes()) }
