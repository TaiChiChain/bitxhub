...
func TestSwarm_OnConnected(t *testing.T) {
	...
	stateLedger.EXPECT().GetState(gomock.Any(), gomock.Any()).DoAndReturn(func(addr *types.Address, key []byte) (bool, []byte) {
		return true, []byte(strings.Join(config.Genesis.Members, ","))
	}).AnyTimes()
	...
	mockLedger.SetState(types.NewAddressByStr(system.NodeMemberAddr), []byte(system.NodeMemberAddr), []byte(strings.Join(config.Genesis.Members, ",")))

	var peerID = "16Uiu2HAmRypzJbdbUNYsCV2VVgv9UryYS5d7wejTJXT73mNLJ8AK"

	success, data := mockLedger.GetState(types.NewAddressByStr(system.NodeMemberAddr), []byte(system.NodeMemberAddr))
	if success {
