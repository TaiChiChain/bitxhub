...
func (gm *GasManager) VotePassExecute(proposal *Proposal) error {
	...
	nextEpochInfo.FinanceParams.MaxGasPrice = types.CoinNumberByMol(extraArgs.MaxGasPrice)
	nextEpochInfo.FinanceParams.MinGasPrice = types.CoinNumberByMol(extraArgs.MinGasPrice)
	nextEpochInfo.FinanceParams.StartGasPrice = types.CoinNumberByMol(extraArgs.InitGasPrice)
	nextEpochInfo.FinanceParams.GasChangeRateValue = extraArgs.GasChangeRateValue
