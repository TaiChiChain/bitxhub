...
func (sa *SmartAccount) Initialize(owner, guardian ethcommon.Address) error {
	...
	if sa.owner.Has() {
		return errors.New("smart account has been initialized")
	}
...
func (sa *SmartAccount) Execute(dest ethcommon.Address, value *big.Int, callFunc []byte) (*big.Int, error) {
	if sa.Ctx.From != ethcommon.HexToAddress(common.EntryPointContractAddr) {
		return big.NewInt(0), errors.New("only entrypoint can call smart account execute")
	}
	...
	if err != nil {
		return big.NewInt(0), fmt.Errorf("execute smart account callWithValue failed: %v", err)
	}
...
func (sa *SmartAccount) ExecuteBatch(dest []ethcommon.Address, callFunc [][]byte) (*big.Int, error) {
	...
	if len(dest) != len(callFunc) {
		return big.NewInt(0), errors.New("dest and callFunc length mismatch")
	}
...
func JudgeOrCallInnerMethod(callData []byte, sa *SmartAccount) (bool, uint64, error) {
	if len(callData) < 4 {
		return false, 0, errors.New("callData length is too short")
	}
	...
	case string(executeSig):
		if len(callData) < 36 {
			return false, 0, errors.New("call smart account execute, callData length is too short")
		}
		var res []any
		res, err = executeMethod.Unpack(callData[4:])
		if err != nil {
			return false, 0, fmt.Errorf("call smart account execute, unpack error: %v", err)
		}
		if len(res) != 3 {
			return false, 0, errors.New("call smart account execute error, unpack result length is not 3")
		}
	...
	case string(executeBatchSig):
		...
		if err != nil {
			return false, 0, fmt.Errorf("call smart account execute batch, unpack error: %v", err)
		}
		if len(res) != 2 {
			return false, 0, errors.New("call smart account execute batch error, unpack result length is not 2")
		}
		dest := res[0].([]ethcommon.Address)
		callFunc := res[1].([][]byte)
	...
	case string(setGuardianSig):
		if len(callData) < 36 {
			return false, 0, errors.New("call smart account set guardian, callData length is too short")
		}
		err = sa.SetGuardian(ethcommon.BytesToAddress(callData[4:36]))
	case string(resetOwnerSig):
		if len(callData) < 36 {
			return false, 0, errors.New("call smart account reset owner, callData length is too short")
		}
		err = sa.ResetOwner(ethcommon.BytesToAddress(callData[4:36]))
	case string(setSessionSig):
		if len(callData) < 132 {
			return false, 0, errors.New("call smart account set session, callData length is too short")
		}
