...
func (c *WhiteList) SetContext(context *common.SystemContractContext) {
...
func (c *WhiteList) Submit(data []byte) error {
	...
	// record log
	c.RecordLog(common.SubmitMethod, b)
	return nil
}
...
func (c *WhiteList) Remove(data []byte) error {
	...
	// record log
	c.RecordLog(common.RemoveMethod, b)
	return nil
}
...
func (c *WhiteList) RecordLog(method string, data []byte) {
	...
	currentLog.Topics = append(currentLog.Topics, types.NewHash(common.Contract2MethodSig[common.WhiteListContractAddr][method]))
