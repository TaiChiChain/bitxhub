	...
	if crit.BlockHash != nil {
		...
		if crit.FromBlock != nil {
			...
			if crit.FromBlock.Int64() == rpc.EarliestBlockNumber.Int64() {
				begin = int64(api.rep.GenesisConfig.EpochInfo.StartBlock)
			}
		}
		...
		filter = NewRangeFilter(api.api, begin, end, crit.Addresses, crit.Topics, api.rep.GenesisConfig.QueryLimitConfig.BlockRangeLimit)
	}
...
func (api *FilterAPI) GetFilterLogs(ctx context.Context, id rpc.ID) ([]*ethereumTypes.Log, error) {
	...
	if f.crit.BlockHash != nil {
		...
		// Construct the range filter
		filter = NewRangeFilter(api.api, begin, end, f.crit.Addresses, f.crit.Topics, api.rep.GenesisConfig.QueryLimitConfig.BlockRangeLimit)
	}
	// Run the filter and return all the logs
	logs, err := filter.Logs(ctx)
	if err != nil {
