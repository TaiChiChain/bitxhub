...
func Bind(types []string, abis []string, fsigs []map[string]string, pkg string, libs map[string]string) (string, error) {
	lang := LangGo
	var (
	...
	for i := 0; i < len(types); i++ {
		...
		for _, original := range evmABI.Methods {
			...
			normalized := original
			normalizedName := methodNormalizer[lang](original.Name)
		...
		for _, original := range evmABI.Errors {
			...
			// Name shouldn't start with a digit. It will make the generated code invalid.
			if len(normalizedName) > 0 && unicode.IsDigit(rune(normalizedName[0])) {
				normalizedName = fmt.Sprintf("Error%s", normalizedName)
				normalizedName = abi.ResolveNameConflict(normalizedName, func(name string) bool {
